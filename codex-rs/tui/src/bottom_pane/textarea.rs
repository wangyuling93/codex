...
impl TextArea {
    ...
    pub fn insert_str(&mut self, text: &str) {
        ...
        if self.replace_selection_with(text).is_some() {
            return;
        }
    ...
    fn current_display_col(&self) -> usize {
        ...
        display_width(&text_for_display(&self.text[bol..self.cursor_pos]))
    }
    ...
    fn render_lines(
        ...
        for (row, idx) in range.clone().enumerate() {
            ...
            for (overlay_range, style) in overlays {
                ...
                let x_off = display_width(&text_for_display(
                    &self.text[line_range.start..overlap_start],
                ));
            ...
            // A logical newline is excluded from the visible slice, but it still occupies a
            // selectable boundary at the end of the visual line. Style the trailing cell so a
            // selection containing only that newline remains visible.
            for (highlight_range, style) in highlights {
                ...
                {
                    let line_width =
                        display_width(&text_for_display(&self.text[line_range.clone()])) as u16;
